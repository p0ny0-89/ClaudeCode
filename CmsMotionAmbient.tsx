...
const GLOW_OPACITY_SMOOTHING = 0.06 // Slower than overlay — glow "lingers"
...
/**
 * CmsMotionAmbient — CMS-friendly hover-reveal card with 3D tilt,
 * slideshow overlay, and ambient lighting.
 *
 ...
 *
 * Ambient lighting renders a blurred, scaled-up duplicate of the
 * active overlay slide behind the card. The glow fades in/out with
 * the overlay but lags slightly for a premium "light lingers" feel.
 *
 * Dual-input slides: bind CMS Image fields to image slots AND
 * CMS File/Video fields to video slots. At runtime, all available
 * media from both sets are collected into a single playlist.
 *
 * Features:
 *   → Dual CMS-bindable media slots (image + video per slide)
 *   → Configurable transitions (instant, fade, slide, push)
 *   → Ambient lighting driven by active overlay content
 ...
 */
export default function CmsMotionAmbient(props: Props) {
    const {
        background,
        backgroundVideo,
        backgroundRadius = 0,
    ...
    // Glow opacity state (lags behind overlay for premium feel)
    const glowOpCurrent = useRef(0)
    ...
    return (
        ...
        >
            ...
            >
                {/* ① Glow layer — behind everything, bleeds past card edges */}
                {ambientEnabled && (hasOverlay || bgSrc) && (
                    <div
                        ref={glowRef}
                        style={{
                            position: "absolute",
                            inset: 0,
                            transform: `scale(${ambientSpread})`,
                            filter: `blur(${ambientBlur}px) saturate(${ambientSaturate}) brightness(${ambientBrightness})`,
                            opacity: 0,
                            pointerEvents: "none",
                            willChange: "opacity",
                        }}
                    >
                        {hasOverlay ? (
                            /* Duplicate slides — always object-fit:cover for uniform glow */
                            <SlidesContent
                                slides={activeSlides}
                            ...
                            /* No slides — use background image for glow */
                            <Media src={bgSrc} />
                        )}
                    </div>
                )}
...
addPropertyControls(CmsMotionAmbient, {
    ...
    ambientEnabled: {
        ...
        disabledTitle: "Off",
        description:
            "Adds a colored glow behind the card based on overlay or background content.",
    },
    ...
    ambientBlur: {
        ...
        step: 1,
        unit: "px",
        hidden: (props: any) => !props.ambientEnabled,
    },
