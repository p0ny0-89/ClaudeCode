...
export const appProductReadiness: ToolcraftProductReadiness = {
  mode: "product",
  productName: "Tile Mural Generator",
  productSummary:
...
export const appAcceptance: readonly ToolcraftComponentAcceptance[] = [
  {
    ...
    browser: true,
    browserTestName: "browser: artwork upload, transform, clear, and reset drive the mural",
    componentType: "fileDrop",
    evidence: "media-lifecycle",
    expectedObservable:
      "Uploading artwork resamples it into per-cell colors and changes the rendered mural pixels; rotate 90° and flip actions update runtime mediaAssets[].transform metadata that the preview renderer and export consume; clear/remove returns the mural to the neutral generated pattern; Reset controls removes the uploaded source image because no default assets exist.",
    fixture:
      "A small high-contrast test image uploaded through the Artwork uploader.",
    id: "artwork.source",
    kind: "control",
    target: "artwork.source",
    userAction:
      "Upload a source image in the Artwork section, rotate it 90°, flip it horizontally, then clear the upload and use Reset controls.",
  },
  {
    automated: true,
    automatedTestName: "artwork placement modes fit fill and repeat map cells differently",
    browser: true,
    browserTestName: "browser: artwork placement mode changes the rendered mural",
    componentType: "segmented",
    evidence: "product-output",
    expectedObservable:
      "Fit letterboxes the artwork inside the wall grid, Fill covers and crops it, Repeat tiles it across the wall; each option produces different sampled cells and different rendered mural pixels.",
    fixture: "An uploaded non-square artwork image so placement differences are visible.",
    id: "artwork.scaleMode",
    kind: "control",
    optionCoverage: "each-visible-item",
    target: "artwork.scaleMode",
    userAction: "Click each Placement segment: Fit, Fill, and Repeat.",
  },
  {
    automated: true,
    automatedTestName: "preview modes draw artwork grid and mural variants",
    browser: true,
    browserTestName: "browser: preview view toggles artwork grid and mural rendering",
    componentType: "segmented",
    evidence: "product-output",
    expectedObservable:
  ...
  {
    automated: true,
    automatedTestName: "artwork size scales placements within the padded area",
    browser: true,
    browserTestName: "browser: artwork size slider drag live-updates the mural",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Dragging Size rescales the artwork placement (a logo can sit smaller than the wall) and the sampled cells and rendered mural update live during the drag.",
    fixture: "An uploaded artwork image with the default Fill placement.",
    id: "artwork.scale",
    kind: "control",
    target: "artwork.scale",
    userAction: "Drag the Size slider thumb.",
  },
  {
    automated: true,
    automatedTestName: "artwork padding keeps clear cells around the artwork",
    browser: true,
    browserTestName: "browser: artwork padding drags discretely and pads the mural",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Dragging the discrete Padding slider keeps empty tile cells clear around the artwork on every side; markers render for each discrete position and the mural updates live during the drag.",
    fixture: "An uploaded artwork image filling the wall.",
    id: "artwork.padding",
    kind: "control",
    target: "artwork.padding",
    userAction: "Drag the Padding slider thumb.",
  },
  {
    automated: true,
    automatedTestName: "repeat spacing separates repeated artwork instances",
    browser: true,
    browserTestName:
      "browser: repeat spacing appears in repeat mode and respaces the mural",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "The Spacing slider is hidden until Placement is Repeat, then becomes visible; dragging the discrete slider inserts tile cells between repeated artwork instances and the mural updates live during the drag.",
    fixture: "An uploaded artwork image with Repeat placement.",
    id: "artwork.spacing",
    kind: "control",
    target: "artwork.spacing",
    userAction:
      "Switch Placement to Repeat so Spacing is shown, drag its thumb, then switch back to Fill so it is hidden.",
  },
  {
    automated: true,
    automatedTestName: "wall unit is recorded in the tile schedule",
    browser: true,
    browserTestName: "browser: wall unit changes the exported schedule unit",
    componentType: "select",
    evidence: "exported-bytes",
    expectedObservable:
      "The selected unit labels every physical dimension in the exported JSON tile schedule; tile counts stay ratio-driven so the grid math is unchanged.",
    fixture: "Default wall and tile dimensions.",
    id: "wall.unit",
    kind: "control",
    optionCoverage: "each-visible-item",
    target: "wall.unit",
    userAction:
      "Choose each unit option (Inches, Feet, Millimeters, Centimeters) and export the JSON schedule.",
  },
  {
    automated: true,
    automatedTestName: "wall width changes the tile column count",
    browser: true,
    browserTestName: "browser: wall width updates the tile grid columns",
    componentType: "text",
    evidence: "product-output",
    expectedObservable:
      "A wider wall adds tile columns: the grid math floor((wall + grout) / (tile + grout)) yields more columns and the rendered mural changes.",
    fixture: "Default 96x72 wall with 4x4 tiles and 0.25 grout.",
    id: "wall.width",
    kind: "control",
    target: "wall.width",
    userAction: "Type a larger Wall width value and commit with Enter.",
  },
  {
    automated: true,
    automatedTestName: "wall height changes the tile row count",
    browser: true,
    browserTestName: "browser: wall height updates the tile grid rows",
    componentType: "text",
    evidence: "product-output",
    expectedObservable:
      "A taller wall adds tile rows and the rendered mural pixels change accordingly.",
    fixture: "Default 96x72 wall with 4x4 tiles and 0.25 grout.",
    id: "wall.height",
    kind: "control",
    target: "wall.height",
    userAction: "Type a larger Wall height value and commit with Enter.",
  },
  {
    automated: true,
    automatedTestName: "tile width changes the tile column count",
    browser: true,
    browserTestName: "browser: tile width updates the mural grid",
    componentType: "text",
    evidence: "product-output",
    expectedObservable:
      "Smaller tile width fits more columns on the same wall; the rendered mural becomes denser.",
    fixture: "Default 96x72 wall.",
    id: "tiles.width",
    kind: "control",
    target: "tiles.width",
    userAction: "Type a smaller Tile width value and commit with Enter.",
  },
  {
    automated: true,
    automatedTestName: "tile height changes the tile row count",
    browser: true,
    browserTestName: "browser: tile height updates the mural grid",
    componentType: "text",
    evidence: "product-output",
    expectedObservable:
      "Smaller tile height fits more rows on the same wall; the rendered mural becomes denser.",
    fixture: "Default 96x72 wall.",
    id: "tiles.height",
    kind: "control",
    target: "tiles.height",
    userAction: "Type a smaller Tile height value and commit with Enter.",
  },
  {
    automated: true,
    automatedTestName: "grout spacing changes tile counts and used wall area",
    browser: true,
    browserTestName: "browser: grout gap changes the rendered mural",
    componentType: "text",
    evidence: "product-output",
    expectedObservable:
      "A larger grout gap widens the visible grout lines between tiles, can reduce tile counts, and changes the rendered mural pixels.",
    fixture: "Default 4x4 tiles on the 96x72 wall.",
    id: "tiles.grout",
    kind: "control",
    target: "tiles.grout",
    userAction: "Type a larger Grout gap value and commit with Enter.",
  },
  {
    automated: true,
    automatedTestName: "grout color fills the wall area behind tiles",
    browser: true,
    browserTestName: "browser: grout color changes the rendered mural",
    componentType: "color",
    evidence: "rendered-pixels",
    expectedObservable:
      "The grout color fills the gaps between tiles and the wall margin behind the tile layout, changing rendered pixels.",
    fixture: "Default mural with visible grout gaps.",
    id: "tiles.groutColor",
    kind: "control",
    target: "tiles.groutColor",
    userAction: "Pick a different Grout color hex value.",
  },
  {
    automated: true,
    automatedTestName: "mapping mode switches between brightness and threshold assignment",
    browser: true,
    browserTestName: "browser: mapping mode reassigns tile modules",
    componentType: "select",
    evidence: "product-output",
    expectedObservable:
      "Brightness maps darker cells to denser modules while Threshold splits cells into solid and empty at the cut-off; switching modes reassigns modules and changes the rendered mural.",
    fixture: "An uploaded artwork image with light and dark regions.",
    id: "mapping.mode",
    kind: "control",
    optionCoverage: "each-visible-item",
    target: "mapping.mode",
    userAction: "Switch Mode between Brightness and Threshold.",
  },
  {
    automated: true,
    automatedTestName: "threshold level splits cells into solid and empty",
    browser: true,
    browserTestName: "browser: threshold slider drag live-updates the mural",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "The Threshold slider is hidden in Brightness mode and becomes visible in Threshold mode; dragging the thumb reassigns solid versus empty cells and the rendered mural changes during the drag, not only on release.",
    fixture: "Threshold mapping mode with uploaded artwork.",
    id: "mapping.threshold",
    kind: "control",
    target: "mapping.threshold",
    userAction:
      "Switch Mode to Threshold so the slider is shown, drag the Threshold thumb, then switch back to Brightness so it is hidden.",
  },
  {
    automated: true,
    automatedTestName: "contrast rescales sampled luminance before module mapping",
    browser: true,
    browserTestName: "browser: contrast slider drag live-updates the mural",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Dragging Contrast pushes sampled luminance away from mid gray, reassigning modules; the rendered mural updates live during the drag.",
    fixture: "An uploaded artwork image with midtones.",
    id: "mapping.contrast",
    kind: "control",
    target: "mapping.contrast",
    userAction: "Drag the Contrast slider thumb.",
  },
  {
    automated: true,
    automatedTestName: "module set switches between mixed and single generation",
    browser: true,
    browserTestName: "browser: module set select changes mural generation",
    componentType: "select",
    evidence: "product-output",
    expectedObservable:
      "Mixed builds each cell from the whole module library; Single repeats one chosen module; switching changes the rendered mural.",
    fixture: "Default mural.",
    id: "modules.mode",
    kind: "control",
    optionCoverage: "each-visible-item",
    target: "modules.mode",
    userAction: "Switch Module set between Mixed modules and Single module.",
  },
  {
    automated: true,
    automatedTestName: "single module mode repeats one chosen module",
    browser: true,
    browserTestName: "browser: single module select becomes visible and changes the mural",
    componentType: "select",
    evidence: "product-output",
    expectedObservable:
      "The Motif select is hidden in Mixed mode and shown in Single mode; every non-empty cell then uses the chosen module and the rendered mural changes per option.",
    fixture: "Single module set on the default mural.",
    id: "modules.single",
    kind: "control",
    optionCoverage: "each-visible-item",
    target: "modules.single",
    userAction:
      "Switch Module set to Single module so the Motif select is visible, then choose different modules; switching back to Mixed hides it.",
  },
  {
    automated: true,
    automatedTestName: "density biases cells toward denser or emptier modules",
    browser: true,
    browserTestName: "browser: density slider drag live-updates the mural",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Dragging Density toward 100% biases cells toward solid modules and toward 0% biases them toward empty; the rendered mural updates live during the drag.",
    fixture: "Default mural.",
    id: "modules.density",
    kind: "control",
    target: "modules.density",
    userAction: "Drag the Density slider thumb.",
  },
  {
    automated: true,
    automatedTestName: "randomness swaps modules within coverage bands",
    browser: true,
    browserTestName: "browser: randomness slider drag live-updates the mural",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Dragging Randomness swaps cells between nearby coverage modules using the seeded stream; the rendered mural updates live during the drag.",
    fixture: "Default mural.",
    id: "modules.randomness",
    kind: "control",
    target: "modules.randomness",
    userAction: "Drag the Randomness slider thumb.",
  },
  {
    automated: true,
    automatedTestName: "seed produces repeatable module assignment",
    browser: true,
    browserTestName: "browser: seed slider drag regenerates a repeatable mural",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Different seeds regenerate different module layouts live during the drag; returning to the same seed reproduces the identical mural.",
    fixture: "Default mural with randomness above zero.",
    id: "modules.seed",
    kind: "control",
    target: "modules.seed",
    userAction: "Drag the Seed slider thumb to a new value and back.",
  },
  {
    automated: true,
  ...
  {
    automated: true,
    automatedTestName: "middle mouse pan matches runtime pan offsets",
    browser: true,
    browserTestName: "browser: middle mouse drag pans the canvas viewport",
    componentType: "canvas",
    evidence: "viewport-side-effect",
    expectedObservable:
      "Holding the middle mouse button and dragging pans the canvas viewport offset exactly like the runtime left-drag/space pan, without touching product values.",
    fixture: "Default mural at 100% zoom.",
    id: "runtime.canvas.middle-pan",
    kind: "runtime",
    userAction: "Press the middle mouse button over the canvas and drag.",
  },
  {
    automated: true,
    automatedTestName: "painted overrides replicate across repeat instances",
    browser: true,
    browserTestName: "browser: paint tool paints a tile and repeats replicate it",
    componentType: "canvas",
    evidence: "product-output",
    expectedObservable:
      "Clicking a tile with the Paint tool fills it with the paint color; in Repeat placement the same pattern-relative tile is painted in every repeated instance, visible in the mural and in the exported schedule's painted cells.",
  ...
  {
    automated: true,
    automatedTestName: "marquee selection adds subtracts and fills tiles",
    browser: true,
    browserTestName:
      "browser: marquee selection with shift and ctrl fills selected tiles",
    componentType: "canvas",
    evidence: "product-output",
    expectedObservable:
      "Dragging a marquee selects the covered tiles, Shift-drag adds to the selection, Ctrl-drag subtracts from it, and Fill selected paints the final selection in the rendered mural.",
    fixture: "Select tool active over the default mural.",
    id: "runtime.canvas.marquee",
    kind: "runtime",
    userAction:
      "Drag a marquee, Shift-drag to add, Ctrl-drag to subtract, then click Fill selected.",
  },
  {
    automated: true,
  ...
  },
  {
    ...
    browser: true,
    browserTestName: "browser: base color changes the rendered mural",
    componentType: "color",
    evidence: "rendered-pixels",
    expectedObservable:
      "The base color fills every tile face behind the accent motif, changing rendered pixels.",
    fixture: "Default mural.",
    id: "colors.base",
    kind: "control",
    target: "colors.base",
    userAction: "Pick a different Base color hex value.",
  },
  {
    automated: true,
    automatedTestName: "accent color fills module motifs",
    browser: true,
    browserTestName: "browser: accent color changes the rendered mural",
    componentType: "color",
    evidence: "rendered-pixels",
    expectedObservable:
      "The accent color fills every module motif shape, changing rendered pixels.",
    fixture: "Default mural with source colors off.",
    id: "colors.accent",
    kind: "control",
    target: "colors.accent",
    userAction: "Pick a different Accent color hex value.",
  },
  {
    automated: true,
    automatedTestName: "source colors sample accents from the artwork",
    browser: true,
    browserTestName: "browser: source colors switch recolors the mural from the artwork",
    componentType: "switch",
    evidence: "product-output",
    expectedObservable:
      "Turning Source colors on makes each motif use the color sampled from the artwork cell instead of the shared accent color, changing the rendered mural.",
    fixture: "An uploaded colorful artwork image.",
    id: "colors.useSource",
    kind: "control",
  ...
  },
  {
    ...
    browser: true,
    browserTestName: "browser: image export format changes exported file type",
    componentType: "select",
    evidence: "exported-bytes",
    expectedObservable:
      "PNG format downloads image/png bytes and JPG format downloads image/jpeg bytes for the same mural.",
    fixture: "Default mural.",
    id: "export.image.format",
    kind: "control",
    optionCoverage: "each-visible-item",
    target: "export.image.format",
    userAction: "Choose PNG and JPG formats and export each.",
  },
  {
    automated: true,
    automatedTestName: "image export resolution changes output pixel dimensions",
    browser: true,
    browserTestName: "browser: image export resolution changes exported dimensions",
    componentType: "select",
    evidence: "exported-bytes",
    expectedObservable:
      "2K, 4K, and 8K produce 2048, 4096, and 8192 pixel long-edge exports through the standard export helper.",
    fixture: "Default mural.",
    id: "export.image.resolution",
    kind: "control",
    optionCoverage: "each-visible-item",
    target: "export.image.resolution",
    userAction: "Choose each Resolution option and export an image.",
  },
  {
    actionCoverage: ["export-json", "export-png"],
    automated: true,
    automatedTestName: "panel export actions build png and json deliverables",
    browser: true,
    browserTestName: "browser: export png and export json deliver product output",
    componentType: "panelActions",
    evidence: "exported-bytes",
    expectedObservable:
      "Export PNG downloads the rendered mural at the selected image resolution with retina-or-larger pixel dimensions and the current background settings; Export JSON downloads a tile schedule with wall, tile, grid, per-tile module, rotation, and color entries; the sticky footer accent indicator is visible while each export Promise is pending and advances with reportProgress.",
    fixture: "Default mural with an uploaded artwork image.",
    id: "export.actions",
    kind: "control",
    target: "export.actions",
    userAction: "Click Export PNG and Export JSON in the sticky footer.",
  },
  {
    automated: true,
    automatedTestName: "resolution scale changes canvas backing pixels",
    browser: true,
    browserTestName:
      "browser: resolution scale drags smoothly and rescales the canvas backing",
    componentType: "slider",
    evidence: "rendered-pixels",
    expectedObservable:
      "Dragging the discrete Resolution scale slider updates the mural canvas backing pixels live during the drag (backing width equals canvas.size.width times the scale) without changing the visible CSS size; markers render for each discrete position.",
    fixture: "Default mural with the runtime Resolution scale slider.",
    id: "canvas.renderScale",
    kind: "control",
    target: "canvas.renderScale",
    userAction: "Drag the Resolution scale slider thumb between 1 and 2.",
  },
  {
    automated: true,
    automatedTestName: "uploaded artwork keeps the current canvas size",
    browser: true,
    browserTestName: "browser: uploading different aspect artwork keeps canvas size",
    componentType: "canvas",
    evidence: "product-output",
    expectedObservable:
      "Uploading artwork with a different aspect ratio keeps the current editable canvas.size unchanged while the artwork is sampled into the wall grid inside the canvas bounds; canvas zoom and offset stay stable.",
    fixture: "A wide artwork image uploaded onto the default 16:9 canvas.",
    id: "runtime.canvas.upload-sizing",
    kind: "runtime",
    userAction: "Upload an artwork image with a non-matching aspect ratio.",
  },
];
...
export const starterControlSectionInventory: readonly ToolcraftControlSectionInventoryEntry[] = [
  {
    ...
    groupingReason:
      "The uploader imports the source artwork; the runtime renders image fileDrop controls as a standalone section.",
    splitReason:
      "Importing the source file and tuning its placement/preview are separate workflow stages, and the runtime renders the image uploader as its own standalone block.",
    targets: ["artwork.source"],
    title: "Artwork",
    workflowStage: "source-import",
  },
  {
    entity: "Artwork placement",
    groupingReason:
      "Placement chooses how the imported artwork maps onto the wall grid, how large it sits, how much clear padding surrounds it, how repeated instances space out, and which interpretation of it the canvas shows.",
    splitReason:
      "Importing the source file and tuning its placement/preview are separate workflow stages, and the runtime renders the image uploader as its own standalone block.",
    targets: [
      "artwork.scaleMode",
    ...
    workflowStage: "placement-preview",
  },
  {
    entity: "Wall surface",
    groupingReason:
      "Physical wall dimensions and their measurement unit define the surface the tile grid is computed from.",
    targets: ["wall.unit", "wall.width", "wall.height"],
    title: "Wall Surface",
  },
  {
    entity: "Tile grid",
    groupingReason:
      "Physical tile dimensions plus grout gap and grout color define the tile grid geometry and the visible joints between tiles.",
    targets: ["tiles.width", "tiles.height", "tiles.grout", "tiles.groutColor"],
    title: "Tile Grid",
  },
  {
    entity: "Artwork-to-tile mapping",
    groupingReason:
      "The mapping mode selector and its dependent threshold plus contrast controls decide how sampled artwork luminance becomes tile fill levels.",
    targets: ["mapping.mode", "mapping.threshold", "mapping.contrast"],
    title: "Tile Mapping",
  },
  {
    entity: "Tile module generation",
    groupingReason:
      "The module set selector and its dependent single-module choice plus density, randomness, and seed tune how fill levels become concrete tile modules.",
    targets: [
      "modules.mode",
      "modules.single",
      "modules.density",
  ...
  },
  {
    ...
    groupingReason:
      "Export format and resolution are one compact workflow pair consumed by the PNG/JPG export helper.",
...
export function validateToolcraftAcceptanceCoverage(
  ...
  // The app inventory only applies to the real app schema; fixture schemas
  // passed by validator self-tests must not inherit this product's sections.
