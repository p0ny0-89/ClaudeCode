...
## Iteration 3 — Floating toolbar, Background removal, Labels view

- Manual paint tools moved to a floating bottom-center toolbar (`src/app/mural-toolbar.tsx`): Pan/Select/Paint icons, an active-tool color swatch opening a popover (native picker, hex field, preset swatches, eyedropper). It is portaled into `[data-slot="toolcraft-runtime-app"]` (inside the React root so events fire, outside `[data-toolcraft-canvas-world]` so the forbidden-canvas-UI scan stays clean and exports are untouched). Paint state is runtime values (`paint.tool`, `paint.color`, `paint.overrides`, `paint.selection`) written via `controls.setValue`; the "Tile Painting" schema section is removed. The eyedropper is a one-shot pick (`paint.tool = "pick"`) that samples a tile and returns to Paint.
- Background section removed end-to-end (schema, renderer, export, acceptance, performance, section inventory). The app-owned acceptance validator gains an `allowMissingOutputBackground` opt-out that defaults on only for `appSchema`; generic validator self-tests still enforce Background for synthetic schemas. PNG export leaves a transparent surround around the wall; JPEG (no alpha) fills the grout color.
- Labels View: a 4th View option (`artwork.previewMode = "labels"`) overlays each tile with its spreadsheet-style coordinate (columns A, B … Z, AA; rows 1, 2 …) drawn on the Canvas 2D surface; the JSON tile schedule gains a `label` field per tile for the install map. See `src/app/mural/labels.ts`.
